...
def get_unprocessed_videos(bucket_name: str) -> tuple:
    ...
    raw_videos = [b.name for b in bucket.list_blobs(prefix="raw/") if any(b.name.lower().endswith(e) for e in VIDEO_EXTENSIONS)]
    if not raw_videos: print(f"   ⚠️ No video files found.")
    print(f"\n🔍 Checking for already processed files in gs://{bucket_name}/processed_json/...")
    processed_jsons = {os.path.splitext(os.path.basename(b.name))[0] for b in bucket.list_blobs(prefix="processed_json/") if b.name.endswith('.json')}
    if not processed_jsons: print("   No processed files found.")
    unprocessed_paths = []
...
def list_bucket_contents(bucket_name: str):
    ...
    if not all_blobs: print("   ⚠️ Bucket is empty!")
    else:
        for blob in all_blobs: print(f"   {blob.name} ({(blob.size or 0) / (1024 * 1024):.2f} MB)")
    print("-" * 60)

def process_video(video_path: str, bucket_name: str, project_id: str, location: str, skip_ocr: bool):
    ...
    cmd = [
        sys.executable, "-m", "scripts.run_ingestion",
        "--video_uri", paths['video_uri'],
...
def main():
    parser = argparse.ArgumentParser(description="Batch process all videos in a GCS bucket /raw folder.")
    parser.add_argument("--bucket_name", required=True, help="GCS bucket name.")
    parser.add_argument("--project_id", required=True, help="GCP Project ID.")
    parser.add_argument("--location", default="global", help="GCP location for Speech-to-Text.")
    parser.add_argument("--skip_ocr", action="store_true", help="Skip OCR extraction on all videos.")
    parser.add_argument("--force_all", action="store_true", help="Process all videos, even if already processed.")
    parser.add_argument("--specific_video", help="Process only a specific video file name (e.g., 'my_video.mp4').")
    parser.add_argument("--debug", action="store_true", help="Show debug information about bucket contents.")
    ...
    try:
        ...
        return
    if args.debug: list_bucket_contents(args.bucket_name)
    if args.specific_video:
        ...
        if args.force_all:
            ...
            videos_to_process = [b.name for b in bucket.list_blobs(prefix="raw/") if any(b.name.lower().endswith(e) for e in VIDEO_EXTENSIONS)]
        ...
        if not videos_to_process:
            ...
            return
    print("\nVideos to process:")
    for video in videos_to_process: print(f"  - {os.path.basename(video)}")
    if not args.specific_video and input("\nProceed with processing? (y/n): ").lower() != 'y':
        print("Cancelled.")
        return
    successful, failed = 0, 0
    for i, video_path in enumerate(videos_to_process, 1):
        print(f"\n--- Processing video {i}/{len(videos_to_process)} ---")
        if process_video(video_path, args.bucket_name, args.project_id, args.location, args.skip_ocr):
            successful += 1
        else:
            failed += 1
    print("\n" + "="*60 + "\n📊 BATCH PROCESSING COMPLETE\n" + "="*60)
    print(f"✅ Successful: {successful}, ❌ Failed: {failed}, 📁 Total: {len(videos_to_process)}")
    print("="*60)
